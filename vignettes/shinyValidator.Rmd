...
Under the hood, `{shinyValidator}` brings its own version of `run_app()` to inject various elements necessary to the validation steps. Your old `run_app()` will be renamed as `run_app-old()`. It also automatically creates a CI/CD file template so that the workflow is ready to run on Github Actions or Gitlab CI/CD. Other elements such as a custom `.lintr` file and package suggests are imported to reduce the amount of work you have to do. Finally, a few JavaScript (JS) assets are copied as they are necessary to run the [Monkey](https://en.wikipedia.org/wiki/Monkey_testing) tests (this avoids proxy issues blocking external JS assets in corporate environments).
...
You have the flexibility to choose whatever level of testing you want. The following code will only enable basic check and linting. 
...
`audit_app()` is the main package function. It essentially runs a series of test based on the predefined parameters or scope:
...
that you have `shinycannon`and a Chrome-based browser. Fortunately, `use_validator()` is able to detect any compatibility issue and accordingly warn you. 
...
If the two previous steps ensure global code quality, it does not check if the application starts properly and remains alive after basic usage. 
...
This step leverages R subprocesses, the idea being to start the app on a child process, connect to the app with a [headless](https://developers.google.com/web/updates/2017/04/headless-chrome) web-browser (without display). The `run_crash-test()` function has two parameters:
  ...
  - __custom_horde__: required if you want to provide custom instructions to manipulate the app with a headless browser. By default, `{shinyValidator}` performs a [Monkey test](https://engineering-shiny.org/build-yourself-safety-net.html#testing-the-interactive-logic) (see also [here](https://github.com/marmelab/gremlins.js/blob/master/README.md)), which consists of triggering random events on the app such as clicks, scroll, input change, ... for 10 seconds. 

```r
shinyValidator::run_crash_test(timeout = 5, custom_horde = NULL)
```

In the following screenshot, since the app is not particularly complex (there is only one slider input), there is no need for a Monkey test. Therefore, we start the app and change the slider input value to 200. This requires to pass a __custom_horde__ parameter. As `{shinyValidator}` relies on `{shinytest2}`, we rely on the `set_inputs` method to manipulate the slider:
...
```r
shinyValidator::record_app(workers = 5, timeout = 5, custom_horde = NULL);
```

__workers__ corresponds to the number of parallel sessions replayed by `shinycannon` in the background. __timeout__ is the time taken the the app to be ready and __custom_horde__ is a parameter to provide custom manipulations. 
...
```r
shinyValidator::upload_reactlog(timeout = 5, headless_actions = NULL);
```
